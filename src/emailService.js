...
// ============================================
// BREVO STATUS ABRUFEN
// ============================================
export async function getEmailStatus(messageId) {
  if (!messageId) return { success: false, error: 'Keine Message ID' };
  
  try {
    const response = await fetch(`https://api.brevo.com/v3/smtp/emails/${messageId}`, {
      method: 'GET',
      headers: {
        'accept': 'application/json',
        'api-key': BREVO_API_KEY,
    ...
    if (!response.ok) {
      return { success: false, error: 'Brevo API Fehler' };
    }
    ...
    return {
      success: true,
      status: data.event, // 'delivered', 'opened', 'clicked', 'bounced', etc.
      events: data.events || [],
      openedAt: data.events?.find(e => e.event === 'opened')?.date || null,
      clickedAt: data.events?.find(e => e.event === 'clicked')?.date || null,
      deliveredAt: data.events?.find(e => e.event === 'delivered')?.date || null,
    };
  } catch (error) {
...
// Status für einen E-Mail Log aktualisieren
export async function refreshEmailStatus(logId) {
  // Log aus DB holen
  ...
  if (!log.brevo_message_id) {
    return { success: false, error: 'Keine Brevo Message ID vorhanden' };
  }

  // Status von Brevo abrufen
  const brevoStatus = await getEmailStatus(log.brevo_message_id);
  
  if (!brevoStatus.success) {
    return brevoStatus;
  }
  ...
  if (brevoStatus.openedAt) {
    updates.opened_at = brevoStatus.openedAt;
  }
  if (brevoStatus.clickedAt) {
    updates.clicked_at = brevoStatus.clickedAt;
  }
  if (brevoStatus.deliveredAt) {
    updates.delivered_at = brevoStatus.deliveredAt;
  }
  if (brevoStatus.status) {
    updates.brevo_status = brevoStatus.status;
  }
...
// Alle E-Mail Status für ein Projekt aktualisieren
export async function refreshAllEmailStatus(projectId) {
  ...
  for (const log of logs) {
    const result = await refreshEmailStatus(log.id);
...
export { getEmailStatus, refreshEmailStatus, refreshAllEmailStatus };
export default { sendEmail, sendWelcomeEmails, sendGoLiveEmail, sendReminderEmail, sendPasswordResetEmail, getEmailStatus, refreshEmailStatus, refreshAllEmailStatus };
